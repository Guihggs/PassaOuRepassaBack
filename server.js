...
io.on("connection", (socket) => {
  ...
  socket.on("team:press", ({ teamId }) => {
    // Aceita o toque enquanto a rodada estiver "live" (botões liberados) ou
    // já "answering" (primeira equipe apurada, mas outras podem ter apertado
    // no mesmíssimo instante). Isso evita perder a ordem real de equipes que
    // apertaram praticamente ao mesmo tempo, só por causa da ordem de chegada
    // dos eventos no servidor.
    if (state.status !== "live" && state.status !== "answering") return;
